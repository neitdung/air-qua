...
function createData(time, ppm) {
    return { time, ppm };
}

let rows = [];

export default function CustomPaginationActionsTable({ url, timeFormat }) {
    ...
    const formatDate = (time) => {
        ...
        if (timeFormat == 'hour') {
            return formattedDate + " " + pad2(hour) + ":00" + "~" + pad2(hour + 1) + ":00";
        }
            ...
            <Table sx={{ minWidth: 100 }} aria-label="custom pagination table">
                <TableBody>
                    {(rowsPerPage > 0
                        ...
                        <TableRow key={formatDate(row.time)}>
